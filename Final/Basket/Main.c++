...
http::response<http::string_body> handle_request(http::request<http::string_body> const& req){
    ...
    if(req.method() == http::verb::get && req.target().starts_with("/basket/")){
        ...
        nlohmann::json json_response = {{"message", Basket::GetBasketById(userId)},
                                        {"URL", "http://www.localhost:8080/games/trade/"}};
        ...
        return res;
    //get all
    }else if(req.method() == http::verb::get && req.target() == "/games/users"){
        ...
        nlohmann::json json_response = {{"basket", basket},
                                        {"URL", "http://www.localhost:8080/basket/"}};
        http::response<http::string_body> res{http::status::ok, req.version()};
        res.set(http::field::server, "Beast");
        res.set(http::field::content_type, "application/json");
        res.keep_alive(req.keep_alive());
    ...
    //for delete by id
        ...
        nlohmann::json json_response = {{"message", "Item deleted"},
                                        {"URL", "http://www.localhost:8080/basket/" + std::to_string(id)}};
        http::response<http::string_body> res{http::status::ok, req.version()};
        res.set(http::field::server, "Beast");
        res.set(http::field::content_type, "application/json");
        res.keep_alive(req.keep_alive());
